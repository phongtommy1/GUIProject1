...
public class Sudoku extends javax.swing.JFrame{
    ...
    public Sudoku(int Score) {
        ...
        getRootPane().getActionMap().put("f1", f1);   //create the ActionMap for the F1 Action (give it the AbstractAction that was created earlier)
    }
        ...
        box.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent key)  //call the keyPresed method and pass a KeyEvent as a parameter
            {
                ...
                if(box.isEditable())  //make sure that the condition check only applies to boxes that are editable (we don't want the hints to be altered)
                {
                    ...
                    {
                        if(c != KeyEvent.VK_BACK_SPACE && c != KeyEvent.VK_ENTER && c != KeyEvent.VK_F1 && c != KeyEvent.VK_ESCAPE)  //also mustcheck if user pressed the backspac or enter key because we will allow those
                        {
